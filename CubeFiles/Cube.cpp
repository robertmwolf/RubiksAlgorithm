...
char green[9];
char yellow[9];
typedef std::vector<char> ArrayList;
ArrayList moveSet;
/*End*/
//----------------------------------
...
{
		cout << "yellowcrossbeginning"<< endl;
		int iteration = 0;
...
int pi() {
	...
	uart0_filestream = open("/dev/ttyACM0", O_RDWR);		//Open in non blocking read/write mode
...
return 1;*/
  std::ofstream arduino;
	...
	//write to it
	//for (auto i = moveSet.begin(); i != moveSet.end(); ++i)
		//arduino << *i;
		
		char move[moveSet.size()];
...
{	
	cout << "start program!";
	//Reads in the cude from a text file.
	readCube(red, orange, yellow, green, blue, white);
	...
	solve_white_cross();
	cout << "whiteCross"<< endl;
	solve_white_corners();
	cout << "whitecorn"<< endl;
	solve_middle_layer();
	cout << "middle solved"<< endl;
	...
	cout << "yellowcorner"<< endl;
	yellow_corner_orientation();
	cout << "yelowcornerorient"<< endl;
	...
	display(yellow);
	/*Jacob*/
	//Prints out set of moves sent to Arduino.
